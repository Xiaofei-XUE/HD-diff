...
Evaluate a trained checkpoint with `test_xin.py`:

```bash
python test_xin.py \
  --data-dir ./data/MICCAI_BraTS2020_TrainingData \
  --checkpoint ./logs_xin/fold0/model/best_model.pt \
  --device cuda:0 \
  --output-csv test_xin_results.csv
```

The test script reports Dice, HD95, inference time, parameter count, and FLOPs when available. Results can be saved to `test_xin_results.csv`.

To skip FLOPs computation:
