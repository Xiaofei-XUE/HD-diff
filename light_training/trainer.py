...
class Trainer:
    ...
    def _should_validate(self, epoch):
        ...
        if isinstance(self.val_every, (list, tuple)):
            assert len(self.val_every) == 3, "val_every schedule should be (switch_epoch, before_interval, after_interval)"
            switch_epoch, before_interval, after_interval = self.val_every
            interval = before_interval if current_epoch <= switch_epoch else after_interval
            return current_epoch % interval == 0
        return current_epoch % self.val_every == 0
